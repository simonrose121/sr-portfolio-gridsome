	...
	<header class="bg-primary m-0 text-white sticky absolute top-0 z-10">
		<div class="container">
			<div class="flex flex-col px-0 justify-between py-4 md:flex-row">
				<div class="flex relative items-center md:w-1/5">
					<g-link to="/" class="w-1/2 md:pl-0 md:w-full">
						<g-image
							alt="Logo"
							:src="
								getStrapiMedia($static.strapi.global.logo.url)
							"
						/>
					</g-link>
					<button
						@click="menuVisible = !menuVisible"
						class="flex absolute right-0 text-xl md:hidden"
					>
						<font-awesome :icon="hamburgerIcon" />
					</button>
				</div>
				<nav
					class="flex bg-primary w-screen h-screen left-0 flex-col align-left gap-6 text-md uppercase mt-8 md:mt-auto md:items-center md:flex md:flex-row md:relative md:justify-end md:align-middle md:w-auto md:h-auto md:l-auto md:text-sm md:gap-4 md:pl-0"
